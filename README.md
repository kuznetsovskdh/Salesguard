...
|---|---|---|
| ![Загрузка](Снимок экрана 2026-08-27 в 12.15.35.png) | ![Результат](Снимок экрана 2026-08-27 в 12.17.57.png) | ![Инсайты](docs/Снимок экрана 2026-08-28 в 09.32.04.png.png) |
...
Проект прошёл стресс-тест на **25 подготовленных файлах** с заранее известными эталонными значениями — не «работает ли код», а «выдерживает ли продукт грязные данные». Найденные и закрытые дефекты:
...
- **Только файловая загрузка** — прямой интеграции с API WB/Ozon нет.
- **Аналитика на уровне товара, не клиента.** Маркетплейсы не передают `customer_id` ни в одном стандартном отчёте, поэтому классический клиентский RFM недоступен by design.
- **Прогноз — линейный бейзлайн.** Он сглаживает недельные пики; для их воспроизведения нужен Prophet, что вынесено в roadmap.
- Автотестов нет: проверка построена на прогоне эталонных файлов с известными значениями.

---

## Roadmap

- [ ] Переход на локальную Ollama в продакшене — данные продавца не покидают контур
- [ ] Prophet для сезонных прогнозов
- [ ] Unit-экономика: расчёт точки безубыточности по SKU
- [ ] Автотесты на pytest поверх эталонных файлов

---

## Автор

**Илья Кузнецов** — разрабатываю проект в рамках перехода в data analytics.
