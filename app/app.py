...
def get_col_debug(df):
    """Return per-column debug info list."""
    result = []
    for col in df.columns:
        ...
        result.append({
            ...
            "confidence": conf, "dtype": dtype,
            "profile": prof_clean, "scores": raw_scores,
        })
    return result
...
def save_debug_profiles(cur, upload_id, debug_cols):
    for d in debug_cols:
        cur.execute('''INSERT INTO debug_profiles
            (upload_id, column_name, mapped_role, confidence, dtype,
             profile_json, scores_json)
            VALUES (%s,%s,%s,%s,%s,%s,%s)''',
            (upload_id, d["column"], d["role"],
             round((d["confidence"] or 0) * 100, 2),
             d["dtype"],
             json.dumps(d["profile"]),
             json.dumps(d["scores"])))

def build_text_log(upload, mapping, debug_cols, verdicts, stats=None):
    lines = []
    lines.append(f"=== Upload #{upload[0]} | {upload[1]} ===")
    lines.append(f"Статус: {upload[6]} | Формат: {upload[2]} | Строк: {upload[4]} | Чистых: {upload[5]}")
    if upload[7]:
        lines.append(f"Ошибка: {upload[7]}")
    lines.append("")
    if stats:
        lines.append(f"--- Статистика ---")
        lines.append(f"Строк: {stats['rows']} | Выручка: {stats['revenue_total']:.0f} | "
    ...
    lines.append("")
    lines.append("--- Debug по колонкам ---")
    for d in debug_cols:
        ...
        top3 = "  scores: " + " | ".join(f"{r}={s}" for r, s in scores_sorted[:4])
        lines.append(top3)
    if verdicts:
...
def build_logs(cur, limit=10):
    cur.execute('''SELECT id,filename,format,rows_total,rows_clean,
    ...
    for u in uploads:
        ...
        logs.append({
            'id':uid,'filename':u[1],'format':u[2],
            'rows_total':rt,'rows_clean':rc,'status':u[5],
            'error_msg':u[6],
    ...
    try:
        ...
        if 'revenue' not in df_std.columns or df_std['revenue'].isna().all():
            ...
            return render_template('result.html',
                charts={}, stats={'rows':0,'revenue_total':0,'avg_check':0,'products':0},
        ...
        for _, row in df.iterrows():
            cur.execute('''INSERT INTO sales
                ...
                (upload_id, safe_val(row.get('sale_date')),
                 safe_val(row.get('product')),
                 safe_val(row.get('quantity')), safe_val(row.get('unit_price')),
                 safe_val(row.get('revenue')), safe_val(row.get('customer_id'))))
...
def debug_history():
    conn = get_db()
    cur = conn.cursor()
    cur.execute('''SELECT u.id,u.filename,u.format,u.rows_total,u.rows_clean,
                   u.status,u.created_at,COUNT(dp.id) as col_count
                   FROM uploads u
                   LEFT JOIN debug_profiles dp ON dp.upload_id=u.id
                   GROUP BY u.id ORDER BY u.created_at DESC LIMIT 50''')
    rows = cur.fetchall()
    conn.close()
    uploads = [{'id':r[0],'filename':r[1],'format':r[2],'rows_total':r[3],
                'rows_clean':r[4],'status':r[5],
                'created_at':r[6].strftime('%d.%m %H:%M') if r[6] else '',
                'col_count':r[7]} for r in rows]
...
def debug_report(upload_id):
    ...
    upload = {'id':u[0],'filename':u[1],'format':u[2],'rows_total':u[4],
              'rows_clean':u[5],'status':u[6],'error_msg':u[7]}

    # Mapping
    cur.execute('''SELECT cm.source_col,cm.mapped_role,cm.confidence,dp.dtype
                   FROM column_mapping cm
                   LEFT JOIN debug_profiles dp
                     ON dp.upload_id=cm.upload_id AND dp.column_name=cm.source_col
                   WHERE cm.upload_id=%s ORDER BY cm.id''', (upload_id,))
    mapping = [{'col':r[0],'role':r[1],'conf':int(r[2] or 0),'dtype':r[3]}
               for r in cur.fetchall()]

    # Debug profiles
    cur.execute('''SELECT column_name,mapped_role,confidence,dtype,
                   profile_json,scores_json
                   FROM debug_profiles WHERE upload_id=%s ORDER BY id''', (upload_id,))
    debug_cols = []
    for r in cur.fetchall():
        debug_cols.append({
            'column': r[0], 'role': r[1],
            'confidence': float(r[2] or 0) / 100,
            'dtype': r[3],
            'profile': json.loads(r[4] or '{}'),
            'scores': json.loads(r[5] or '{}'),
        })

    # Verdicts
    cur.execute('''SELECT column_name,row_index,reason
                   FROM value_verdicts WHERE upload_id=%s ORDER BY id''', (upload_id,))
    verdicts = [{'col':r[0],'row':r[1],'reason':r[2]} for r in cur.fetchall()]

    # Stats from sales
    cur.execute('''SELECT COUNT(*),SUM(revenue),AVG(revenue),COUNT(DISTINCT product)
                   FROM sales WHERE upload_id=%s''', (upload_id,))
    s = cur.fetchone()
    stats = None
    if s and s[0]:
        stats = {'rows': s[0] or 0,
                 'revenue_total': float(s[1] or 0),
                 'avg_check': float(s[2] or 0),
                 'products': int(s[3] or 0)}
    ...
    return render_template('debug_report.html',
                           upload=upload, mapping=mapping,
                           debug_cols=debug_cols, verdicts=verdicts,
                           stats=stats, text_log=text_log)

@app.route('/debug-page')
def debug_page():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("""SELECT u.id,u.filename,u.status,u.created_at
                   FROM uploads u ORDER BY u.created_at DESC LIMIT 30""")
    ...
    return render_template('debug.html', filename='', report=[],
                           history=hist, log=None, mapping=[],
                           stats=None, verdicts=[], text_log='')
...
def debug():
    ...
    try:
        ...
        has_revenue = ('revenue' in df_std.columns and
                       not df_std['revenue'].isna().all())
        stats = None
        rows_clean = 0
        ...
        for col, verdicts_list in matrix.items():
    ...
    if 'matrix' in dir():
        for col, vlist in matrix.items():
            for i, v in enumerate(vlist):
                if not v.is_valid:
                    verdicts_disp.append({'col':col,'row':i,'reason':v.reason})
    ...
    drop_pct = round(dropped/rt*100,1) if rt else 0
    log = {
        'filename': file.filename, 'format': ext,
    ...
    # Text log
    u_raw = (upload_id or 0, file.filename, ext, None, rt, rc, status, err_msg)
    text_log = build_text_log(u_raw, mapping, debug_cols, verdicts_disp, stats)

    # History
    cur.execute("""SELECT id,filename,status,created_at
                   FROM uploads ORDER BY created_at DESC LIMIT 30""")
    hist = [{'id':r[0],'filename':r[1],'status':r[2],
             'created_at':r[3].strftime('%d.%m %H:%M') if r[3] else ''}
            for r in cur.fetchall()]
    conn.close()
    ...
    return render_template('debug.html',
                           filename=file.filename, report=debug_cols,
                           history=hist, log=log, mapping=mapping,
                           stats=stats, verdicts=verdicts_disp,
                           text_log=text_log)
